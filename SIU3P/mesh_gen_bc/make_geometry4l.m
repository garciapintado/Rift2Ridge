function [GEOMETRY, Geo_id] = make_geometry(x_min, x_max,y_min1, y_max1, y_max2, y_max3, y_max4, cont_points, ini_interf, Rho, sigma_moho, topo_moho)

%interfaces
xx_moho = linspace(x_max,x_min,cont_points(5));

switch ini_interf
    case {0,5} % No initial topography for any interfaces
        moho = y_max2*ones(1,cont_points(5));
        
        surface = y_max4*ones(1,cont_points(9));
    case 1 % Sinusoidal negative topography only for the Moho
        moho = y_max2 + topo_moho*exp(-xx_moho.^2./(sigma_moho^2));
        moho(1) = y_max2;
        moho(end) = y_max2;
    ...
    case 2 % Sinusoidal negative topography for the Moho and resulting 
        ...
        surface_m = (moho-y_max2)*(Rho(3)-Rho(2))/Rho(4);
        surface = interp1(linspace(x_max,x_min,cont_points(5)), ...
            surface_m, linspace(x_max,x_min,cont_points(9)));
...
LAYER3_3    = [ linspace(x_max,x_min,cont_points(7)); y_max3*ones(1,cont_points(7))];
...
%layer4
LAYER4_2    = [ x_max*ones(1,cont_points(8))    ; linspace(y_max3,y_max4,cont_points(8))];
LAYER4_2(:,[1 end]) = [];
Geo_id      = [Geo_id 11*ones(1, size(LAYER4_2,2))];
LAYER4_3    = [ linspace(x_max,x_min,cont_points(9)); surface];
Geo_id      = [Geo_id 12*ones(1, size(LAYER4_3,2))];
LAYER4_4    = [x_min*ones(1,cont_points(8))          ; linspace(y_max4,y_max3,cont_points(8))];
LAYER4_4(:,[1 end]) = [];
