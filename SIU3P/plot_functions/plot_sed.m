...
% Sorts SED.ISOCHRONS
ISOCHRONS_sort = SED.ISOCHRONS(:,ind_plot,:);
...
for jp = 1:length(t_horizon)
    ...
    % Topography of the step_horizon isochron
    Sediment_p = ISO_TOPO(1,:,floor(step_horizon));
    
    % Removes the eroded sediments
    Xp = [X_basement_pr(1) X_basement_pr X_basement_pr(end)];
    Yp = [max_sed_topo Sediment_p max_sed_topo];
    patch(Xp/1000,Yp/1000,jp*ones(size(Xp)),'w','EdgeColor','none')
    
    % Plot horizon
    plot3(X_basement_pr/1000,Sediment_p/1000,jp*ones(size(X_basement_pr)),'k')
end
