...
for i=1:nel
    ...
    EL2N(i,:) = is:ie;
    Dummy      = Nbig'\SIGMA_xx(i,:)';
    SIGMAxx_n(:,i)= Dummy(1:3);
end
...
% Plot strain rate
% ----------------
title(['xx component of the Cauchy stress (',num2str(istep*dt/ma),' Myr)'])
xlabel('Distance [km]')
ylabel('Depth [km]')
...
if exist('log_data','var')
    if log_data
        patch('faces',EL2N,'vertices',GCOORD_N'/1000,'facevertexcdata', ...
            log10(SIGMAxx_n(:)),'FaceColor','flat')
    else
        patch('faces',EL2N,'vertices',GCOORD_N'/1000,'facevertexcdata', ...
            SIGMAxx_n(:),'FaceColor','flat')
    end
else
    patch('faces',EL2N,'vertices',GCOORD_N'/1000,'facevertexcdata', ...
        SIGMAxx_n(:),'FaceColor','flat')
...
if exist('log_data','var')
    if log_data
        title(hc,'$log(\sigma_{xx})$','interpreter','latex', ...
            'FontSize',10)
    end
else
    title(hc,'$\sigma_{xx}$','interpreter','latex', ...
        'FontSize',10)
end
hold on
