function [GCOORD,Temp,F_xx,F_xy,F_yx,F_yy,I2,TAU_xx_old,TAU_xy_old, ...
    TAU_yy_old,Mu_all,E2all,remesh] = update_topo_values(GCO,ELEM2NODE, ...
    NEW_TOPO,Topo2nodes,Temp,temp_surf,F_xx,F_xy,F_yx,F_yy,I2,RHEOL, ...
...
% Interpolate
[Fxx,Fxy,Fyx,Fyy,~,~,I,Txx,Txy,Tyy] = remesh_F_TAU ...
    (GCOORD,ELEM2NODE(:,reshape_el),GCO,ELEM2NODE(:,reshape_el), ...
    Fxx,Fxy,Fyx,Fyy,nnodel_r,Txx,Txy,Tyy,nip,length(reshape_el));

% Viscosity
Mu_old = Mu_all(reshape_el,:);
Mu = remesh_val_ip(GCO,ELEM2NODE(:,reshape_el),Mu_old,nip,nip,GCOORD, ...
    ELEM2NODE(:,reshape_el));
Mu(Mu<0) = -Mu(Mu<0);

% Strain rate invariant
E_old = E2all(reshape_el,:);
E = remesh_val_ip(GCO,ELEM2NODE(:,reshape_el),E_old,nip,nip,GCOORD, ...
    ELEM2NODE(:,reshape_el));
...
% Gradient of deformation (full deformation since sediments are considered
% weak)
Fxx(Tris_ip==0) = 1;
F_xx(reshape_el,:) = Fxx;
Fxy(Tris_ip==0) = 0;
...
% Historic second invariant of the strain (full deformation since sediments
% are considered weak)
I(Tris_ip==0) = 0;
I2(reshape_el,:) = I;
