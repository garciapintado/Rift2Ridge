function [TopoXY,SP] = diff_topo_sealand_NL(TopoXY,Basement,GCOORD,SP,dt,ma)
% NONLINEAR VERSION OF diff_topo_sealand (NOT WORKING)
...
%         plot(TopoXY(1,sbm_ss_seg~=0),TopoXY(2,sbm_ss_seg~=0),'o')
    ...
    Rhs_sed = sed_source*ones(size(Topography)); % TODO this is crap
    B = interp1(Basement(1,:),Basement(2,:),TopoXY(1,:));
    sed_thick = B'-Topography;
    sed_thick(sed_thick<0) = 0;
...
%     Topography = Topography + tpelagic';
    ...
    Topography = function_sealand_NL ...
            (Topography,no_nodes_sed,Rhs_sed,MM_SED,De,SP.nexp,dt_sed, ...
            kappa,Dx_sed,sealevel,ksea,kappa_s,kdecay,sed_thick,2,2);
