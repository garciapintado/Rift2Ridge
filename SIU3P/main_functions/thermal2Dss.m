function T = thermal2Dss(ELEM2NODE, GCOORD, Temp, ...                      % T,Temp: [1,nnod7]
                         RhoCp, Hp, Cond, Hs, ...                           % [nel,nip] variables
                         Bc_ind, Bc_val, dt, reorder, nelblk, steady_state)
% THERMAL2D Two dimensional finite element thermal problem solver of rift2ridge2D
% based on the MILAMIN approach
%
...
% 13. steady_state :: LOGICAL, OPTIONAL false (default) for transient solution

% Javier Garcia-Pintado: included the optional state state solution
...
% MODEL INFO
%==========================================================================
nnod   = max(ELEM2NODE(:));                                                % number of nodes
...
%==========================================================================
[x_ip, IP_w] = ip_triangle(nip);                                           % IP_X [nip,2], IP_w [1,nip] with nip=6; 
...
for ib = 1:nblk
    ...
    ECOORD_x = reshape( GCOORD(1,ELEM2NODE(1:nvert,els_blk)), nvert, nelblk);     % REAL [nnodel, nelblk]
    ECOORD_y = reshape( GCOORD(2,ELEM2NODE(1:nvert,els_blk)), nvert, nelblk);     % REAL [nnodel, nelblk]
    ...
    ED = Cond(els_blk,:)';                                                 % [nip,nelblk] thermal conductivity
    F = (Hp(els_blk,:) + Hs(els_blk,:))';                                  % [nip,nelblk] heat source                   
    
    if ~steady_state
        ED = dt .* ED;
        F  = dt .* F;
        ET = RhoCp(els_blk,:)';                                            % [nip,nelblk] 
    end
    %==============================================================
    ...
    for ip=1:nip    
        ...
        %dQdt_blk  = Hp(els_blk,ip)' + Hs(els_blk,ip)';                     % REAL [1,nelblk] radiogenic + shear heat production
        
        indx = 1;                                                          % indx: column in extended stiffness matrix
        for i = 1:nnodel
            for j = i:nnodel
                K_blk(indx,:) = K_blk(indx,:) ...                                               % extended stiffness for transient
                    + (dNdx(i,:).*dNdx(j,:) + dNdy(i,:).*dNdy(j,:)) .* (ED(ip,:) .* weight_ip); % stiffness matrix term      dt * cond_i * dNxdN * |J_i| * w_i
                if ~steady_state
                    K_blk(indx,:) = K_blk(indx,:) + NxN(i,j) .* (ET(ip,:) .* weight_ip);        % mass matrix term           rho_i * cp_i * NxX  * |J_i| * w_i        
                end
                indx = indx + 1;
            end
        end
        ...
        Rhs_blk  = Rhs_blk ...                                             % [nnodel, nelblk]
            + Ni * (F(ip,:) .* weight_ip);                                 % [nnodel, nelblk] <- [nnodel,1].[1,nelblk] heat source
        if ~steady_state
            Rhs_blk  = Rhs_blk ...                                             % [nnodel, nelblk]
            + Ni * ((Ni'*T_blk) .* ET(ip,:) .* weight_ip);                     % previous temperature
        end    
    end
    %==============================================================
