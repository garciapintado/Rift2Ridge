...
FS = TopoXY(1,:);
FS(:) = 0;
FS(TopoXY(1,:)==min(abs(TopoXY(1,:)))) = -0.05/ma*1e6;
...
if SP.scale
    ...
    q_bc = SP.q_bc;
    lriver = SP.lriver;
    Rhs_sed = zeros(size(TopoXY));  % If we ever switch to souce then
        % we will need here: 'Rhs_sed = SP.source;' and also don't update
        % topographies with the velocities
end
...
for j = 1:ntsteps_sed
    ...
    if ksea~=0
        ...
        for n = 1:size(nseg,2)
            ...
            % Remesh source term
            Rhs_slope = interp1(TopoXY(1,seg_nodes),Rhs_sed(seg_nodes),xslope);
            ...
            Rhs_sed = [Rhs_sed(ileft) Rhs_slope Rhs_sed(iright)];
...
%         plot(TopoXY(1,sbm_ss_seg~=0),TopoXY(2,sbm_ss_seg~=0),'o')
    ...
    FS = interp1(Old_topo(1,:),FS,TopoXY(1,:))';
    Rhs_sed = FS';
    FS(:) = 0;
    % Calculating new topography
    Topography = function_sealand_new ...
            (Topography,no_nodes_sed,Rhs_sed,MM_SED,De,SP.nexp,dt_sed, ...
            kappa,Dx_sed,sealevel,ksea,kappa_s,kdecay,q_bc,FS,lriver,2,2);
