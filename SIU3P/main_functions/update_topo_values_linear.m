function [GCOORD,Temp,F_xx,F_xy,F_yx,F_yy,I2,TAU_xx_old,TAU_xy_old, ...
    ...
    NEW_TOPO,Topo2nodes,Temp,temp_surf,F_xx,F_xy,F_yx,F_yy,I2, ...
    TAU_xx_old,TAU_xy_old,TAU_yy_old,Mu_all,E2all, PHY, R,Phases,remesh)
...
change_topo = GCOORD(2,Topo2nodes)~=NEW_TOPO(2,:);
NODES_CT = Topo2nodes(change_topo);
...
%==========================================================================
% Update topography
GCOORD(2,Topo2nodes) = NEW_TOPO(2,:);
% Make straight edges
...
%==========================================================================
% CHECK FOR NODES ABOVE TOPOGRAPHY
%==========================================================================
Not_topo = ~ismember(1:size(GCOORD,2),Topo2nodes);
NEW_TOPO2GCO_Y = interp1(NEW_TOPO(1,:),NEW_TOPO(2,:),GCOORD(1,Not_topo));
if sum(GCOORD(2,Not_topo)>NEW_TOPO2GCO_Y)>0
    remesh = 1;
end
...
[GIPold_x,GIPold_y] = ...
    ip_coord(GCO,ELEM2NODE(:,reshape_el),length(reshape_el),6);
...
if PHY.SS.rI2 == "triscatteredinterp"
    Fc      = scatteredInterpolant(GIPold_x(:),GIPold_y(:),I.c(:), ...
        'natural','linear');
    Itsi_c  = Fc(GIP_x,GIP_y);
    Fp      = scatteredInterpolant(GIPold_x(:),GIPold_y(:),I.p(:), ...
        'natural','linear');
    Itsi_p  = Fp(GIP_x,GIP_y);
end
...
%==========================================================================
% Temperature
Temp(nindx(Tris_n==0)) = temp_surf;

% Gradient of deformation (no deformation since sediments are considered
% underformed)
Fxx(Tris_ip==0) = 1;
F_xx(reshape_el,:) = Fxx;
Fxy(Tris_ip==0) = 0;
...
% Historic second invariant of the strain (no deformation since sediments
% are considered undeformed)
if PHY.SS.rI2 == "triscatteredinterp"
    I.f(Tris_ip==0) = 0;
    I2.f(reshape_el,:) = I.f;
    Itsi_p(Tris_ip==0) = 0;
    I2.p(reshape_el,:) = Itsi_p;
    Itsi_c(Tris_ip==0) = 0;
    I2.c(reshape_el,:) = Itsi_c;
else       % "linear_N"
    I.f(I.f<0) = 0;
    I.p(I.p<0) = 0;
    I.c(I.c<0) = 0;
